...
which node >/dev/null && success "[+] Node already installed" || (brew install node >/dev/null 2>&1 && nodeVersion=`node -v` && npmVersion=`npm -v` && success "[+] Installed node" || error "[-] Failed to install node")

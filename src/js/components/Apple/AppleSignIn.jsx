  ...
  signInToApple () {
    ...
    if (version) {
      ...
      if (floatVersion < 13.0) {
        ...
        $('#appleLogo').css('color', 'grey');
        $('#appleSignInText').css('color', 'grey');
        return;
      }
    } else {
      console.log('Sign in with apple not able to determine iOS version', version);
      return;
    }
    ...
    if (signedIn) {
      ...
      return (
        <AppleSignInContainer>
          <AppleSignInButton type="submit" isWeb={isWeb} onClick={() => this.signInToApple()}>
            {logo}
            <AppleSignInText id="appleSignInText">
              Sign in with Apple
            </AppleSignInText>
          </AppleSignInButton>
        </AppleSignInContainer>

...
export default class ReadMore extends Component {
  ...
  render () {
    ...
    let {
      text_to_display, link_text, num_of_lines, collapse_text,
    } = this.props;
    ...
    text_to_display = text_to_display.replace(/[\x0D-\x1F]/g, ""); // eslint-disable-line no-control-regex
    // convert text into array, splitting on line breaks
    const expanded_text_array = text_to_display.replace(/(?:\r\n|\r|\n){2,}/g, "\r\n\r\n").split(/(?:\r\n|\r|\n)/g);
    ...
    for ( let i = 0; i < max_num_of_lines; i++) {
    ...
    const expanded_text_to_display = expanded_text_array.map((item, key) => {
      if (key === 0) {
        return (
          <span key={key}>
            {item}

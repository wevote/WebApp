...
class MeasureStore extends ReduceStore {
  ...
  getYesVoteDescription (measure_we_vote_id) {
    if (measure_we_vote_id) {
      const measure = this.getMeasure(measure_we_vote_id);
  ...
  getPositionAboutMeasureFromOrganization (measure_we_vote_id, organization_we_vote_id) {
    const positions_about_measure = this.getState().all_cached_positions_about_measures[measure_we_vote_id] || [];
  ...
  reduce (state, action) {
    ...
    switch (action.type) {
      ...
      case "voterGuidesToFollowRetrieve":
        ...
        return {
          ...state,
          all_cached_positions_about_measures,
        };
